...
export async function mergeData() {
    ...
    for (const item of arr) {
        ...
        if (gateway.length) {
            for (const agency of gateway) {
                ...
                input.forEach((item) => {
                    ...
                    const existingChapter = output.find(
                        (chapter) => chapter.chapterName === item.Chapter,
                    );
        ...
        for (const { fields } of entity) {
            ...
            archicad = [
                ...archicad,
                ...fields["Archicad Representation"].split(",").map((x) => x.trim()),
            ];
            tekla = [...tekla, ...fields["Tekla Representation"].split(",").map((x) => x.trim())];
            bentley = [
                ...bentley,
                ...fields["Bentley Representation"].split(",").map((x) => x.trim()),
            ];
        }
    ...
    fs.writeFileSync(
        "./../src/routes/(main)/identified-component/mg_comp.json",
    ...
    fs.writeFileSync(
        "./../src/routes/(main)/api/ifcsg/get-code/rules.json",
